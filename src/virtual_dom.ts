...
export const headingsToVirtualHeaderRowDOM = (
    ...
    {classes, format, hiddenHeader, sortable, scrollY, type}: DataTableOptions,
    {noColumnWidths, unhideHeader}: renderOptions
) => ({
    nodeName: "TR",

    childNodes: headings
        .map((heading: headerCellType, index: number): elementNodeType | void => {
            const column =
            ...
            const headerNodes: elementNodeType[] =
                heading.type === "html"
...
export const dataToVirtualDOM = (
    tableAttributes: {[key: string]: string},
    headings: headerCellType[],
    rows: rowType[],
    columnSettings: columnSettingsType[],
    columnsState: columnsStateType,
    rowCursor: number | false,
    {classes, hiddenHeader, header, footer, format, sortable, scrollY, type, rowRender, tabIndex}: DataTableOptions,
    {noColumnWidths, unhideHeader, renderHeader}: renderOptions,
    footers: elementNodeType[],
    captions: elementNodeType[]
) => {
        ...
        childNodes: [
            {
                ...
                childNodes: rows.map(({row, index}) => {
                    const tr: elementNodeType = {
                        ...
                        childNodes: row.cells
                            .map((cell: cellType, cIndex: number) => {
                                const column =
                                    columnSettings[cIndex] ||
                                    ({
                                        type,
                                        format,
                                        sortable: true,
                                ...
                                if (!header && !footer && columnsState.widths[cIndex] && !noColumnWidths) {
                                    td.attributes.style = joinWithSpaces(
                                        td.attributes.style,
                                        `width: ${columnsState.widths[cIndex]}%;`
                                    )
                                }
                                if (column.cellClass) {
                                ...
                                if (column.render) {
                                    const renderedCell: string | elementNodeType | void = column.render(
                                        cell.data,
                                        td,
                                        index,
                                        cIndex
                                    ...
                                    if (renderedCell) {
                                        if (typeof renderedCell === "string") {
                                            ...
                                            if (
                                                ...
                                                !["#text", "#comment"].includes(node.childNodes[0].nodeName)
                                            ) {
    ...
    if (header || footer || renderHeader) {
        const headerRow: elementNodeType = headingsToVirtualHeaderRowDOM(
            headings,
            columnSettings,
            columnsState,
            {classes, hiddenHeader, sortable, scrollY},
